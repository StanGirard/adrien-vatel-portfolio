...
export function ProjectGrid({
  ...
  return (
    <>
      ...
      <div
        id={gridId}
        className="px-4 md:px-12 flex border-b-[3px] border-black bg-off-white overflow-x-auto"
      >
        {filters.map((f) => (
          <button
            key={f}
            onClick={() => setActiveFilter(f)}
            className="bg-transparent border-none font-mono text-[9px] tracking-[0.2em] uppercase cursor-pointer px-4 md:px-6 py-4 transition-all -mb-[3px] whitespace-nowrap"
            style={{
        ...
        {projects.map((p) => {
          ...
          return (
            <div
              ...
              className={p.featured ? "col-span-1 md:col-span-2" : ""}
            >
              <ProjectCard
                project={p}
